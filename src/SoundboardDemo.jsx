...
import { Search, Heart, Plus, X, ChevronLeft, User, ListMusic, Check, Settings, Pencil, Bell, Frown, MessageCircle, Send, CornerUpLeft, UserPlus, AtSign, Home, Compass, Layers } from "lucide-react";
    ...
    <div style={{ fontFamily: "Helvetica Neue, Helvetica, Arial, sans-serif", background: BG, minHeight: "100%", color: INK, overflowX: "hidden", maxWidth: "100vw" }}>
      ...
      {IS_NATIVE_APP && (
        <>
          ...
          <div style={{ position: "fixed", left: 0, right: 0, bottom: 0, zIndex: 80, background: BG, borderTop: `1px solid ${INK}`, paddingBottom: "env(safe-area-inset-bottom, 0px)", display: "flex", alignItems: "stretch", justifyContent: "space-around" }}>
            {[
              { key: "home", label: "home", Icon: Home, active: view.name === "home", go: () => setView({ name: "home" }) },
              { key: "browse", label: "browse", Icon: Compass, active: ["browse", "tagResults", "artist"].includes(view.name), go: () => setView({ name: "browse" }) },
              { key: "create" },
              { key: "mixes", label: "mixes", Icon: Layers, active: ["mixes", "albumMixDetail", "songMixDetail"].includes(view.name), go: () => setView({ name: "mixes" }) },
              { key: "profile", label: "profile", Icon: User, active: view.name === "profile", go: () => setView({ name: "profile" }) },
            ].map((t) => t.key === "create" ? (
              <button key="create" onClick={() => setShowCreateSheet((s) => !s)} aria-label="create" style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", padding: "6px 0", flex: 1 }}>
                <div style={{ width: 44, height: 44, background: INK, color: BG, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <Plus size={22} strokeWidth={2.2} />
                </div>
              </button>
            ) : (
              <button key={t.key} onClick={t.go} className="ui-sans" style={{ background: "none", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3, padding: "8px 0", flex: 1, color: t.active ? BLUE : MUTE, fontFamily: "inherit" }}>
                <t.Icon size={21} strokeWidth={1.9} />
                <span style={{ fontSize: 10.5, fontWeight: t.active ? 600 : 400 }}>{t.label}</span>
              </button>
            ))}
          </div>
        </>
