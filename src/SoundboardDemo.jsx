...
// True only inside the native Capacitor app (iOS/Android). Constant at runtime —
// used to show the fixed bottom tab bar ONLY in the app, never on mobile web.
...
// ---- STATS MASCOT: stills vs. animation ------------------------------------
// Set USE_STATS_STILLS = false to restore the original animated SVG mascot.
// Nothing was removed to make this work — the animated code below is intact
// and runs again the moment this flag flips back.
const USE_STATS_STILLS = true;
const STATS_STILL_SRC = {
  grin:    "/stats/mascot-grin.png",
  smile:   "/stats/mascot-smile.png",
  neutral: "/stats/mascot-neutral.svg",
  frown:   "/stats/mascot-frown.png",
  dead:    "/stats/mascot-dead.png",
};
// ---------------------------------------------------------------------------
    ...
    <div className="ui-sans" style={{ textAlign: "left" }}>
      ...
      {loading ? (
        ...
        <div style={{ fontSize: 13.5, color: MUTE, padding: "24px 0", lineHeight: 1.6 }}>
          {isOwn
            ? "nothing here yet — reviews, posts, likes and comments you make will show up here."
          ...
          {groups.map((g) => (
            <div key={g.label}>
              ...
              {g.items.map((it) => {
                ...
                if (it.type === "review") {
                  const url = it.coverArtUrl ? it.coverArtUrl.replace("http://", "https://") : null;
                  return (
                    <div key={`review-${it.id}`} onClick={() => { if (it.albumId && onOpenAlbum) onOpenAlbum(it.albumId); }} style={{ display: "flex", gap: 12, alignItems: "center", padding: "14px 0", borderTop: `1px solid #ededea`, cursor: "pointer" }}>
                      <div style={{ width: 44, height: 44, flexShrink: 0, background: url ? "transparent" : "#eee", overflow: "hidden" }}>
                        {url ? <img src={url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} /> : null}
                      </div>
...
function StatsMascot({ mood = "neutral", size = 140, motion = "none", color }) {
  ...
  if (USE_STATS_STILLS) {
    const stillSrc = STATS_STILL_SRC[mood] || STATS_STILL_SRC.neutral;
    return (
      <div
        role="img"
        aria-label="noteblock mascot"
        style={{
          width: size,
          height: size,
          display: "block",
          flexShrink: 0,
          background: B,
          WebkitMaskImage: `url(${stillSrc})`,
          maskImage: `url(${stillSrc})`,
...
export default function SoundboardDemo() {
  ...
  // Selected palette for the admin-only "bubble" theme (persisted client-side).
  const [bubblePalette, setBubblePalette] = useState(() => { try { return localStorage.getItem("nb_bubble_palette") || "aqua"; } catch (e) { return "aqua"; } });
  function chooseBubblePalette(k) { setBubblePalette(k); try { localStorage.setItem("nb_bubble_palette", k); } catch (e) {} }
  ...
  const [showCreateSheet, setShowCreateSheet] = useState(false); // mobile bottom-nav create menu
...
apiFetch(`${BACKEND_URL}/api/mixes/saved`)
              ...
              <div style={{ marginTop: 18, border: `1px solid ${LINE}`, borderRadius: 0, padding: 18 }} className="ui-sans">
                ...
                <div style={{ borderTop: `1px solid ${LINE}`, marginTop: 20, paddingTop: 18, textAlign: "left" }}>
                  ...
                  <div style={{ marginTop: 22 }}>
                    ...
                    <select
                      value={profile.profileTheme === "bubble" ? "" : (profile.profileTheme || "")}
                  ...
                  {profile.username === ADMIN_USERNAME && (
                    <div style={{ marginTop: 22, borderTop: `1px solid ${LINE}`, paddingTop: 18 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: "#6b6b74", letterSpacing: "0.01em" }}>bubble <span style={{ fontWeight: 400, color: MUTE }}>(admin test)</span></div>
                      <div style={{ fontSize: 11, color: MUTE, marginTop: 4, marginBottom: 12 }}>frutiger aero theme — pick a palette to apply it</div>
                      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                        {Object.keys(AERO_PALETTES).map((k) => {
                          const on = profile.profileTheme === "bubble" && bubblePalette === k;
                          return (
                            <button
                              key={k}
                              onClick={() => { chooseBubblePalette(k); if (profile.profileTheme !== "bubble") saveProfileTheme("bubble"); }}
                              title={AERO_PALETTES[k].label}
                              aria-label={`Bubble ${AERO_PALETTES[k].label} palette`}
                              style={{ width: 34, height: 34, borderRadius: 9999, background: AERO_PALETTES[k].swatch, cursor: "pointer", padding: 0, border: on ? `2px solid ${INK}` : "1px solid rgba(0,0,0,.15)", outline: on ? "2px solid #fff" : "none", outlineOffset: -4, boxShadow: on ? `0 0 0 1px ${INK}` : "none" }}
                            />
                          );
                        })}
                        {profile.profileTheme === "bubble" && (
                          <button className="sb-btn" onClick={() => saveProfileTheme("")} style={{ marginLeft: 6 }}>turn off</button>
                        )}
                      </div>
                    </div>
                  )}
  ...
  return (
    ...
    <div style={{ fontFamily: "Helvetica Neue, Helvetica, Arial, sans-serif", background: BG, minHeight: "100%", color: INK, overflowX: "hidden", maxWidth: "100vw" }}>
      ...
      {/* NATIVE-APP BOTTOM NAV — fixed tab bar, only inside the Capacitor app.
          Mobile web and desktop keep the top nav. */}
      {IS_NATIVE_APP && (
        <>
          {showCreateSheet && (
            <div onClick={() => setShowCreateSheet(false)} style={{ position: "fixed", inset: 0, zIndex: 90, background: "rgba(0,0,0,0.3)" }}>
              <div onClick={(e) => e.stopPropagation()} className="ui-sans" style={{ position: "fixed", left: 12, right: 12, bottom: "calc(70px + env(safe-area-inset-bottom, 0px))", zIndex: 91, background: BG, border: `1px solid ${LINE}`, borderRadius: 0, overflow: "hidden", boxShadow: "0 6px 24px rgba(0,0,0,0.18)" }}>
                {[
                  { label: "write a thought", desc: "share what's on your mind", action: () => { setShowCreateSheet(false); setShowTextPostModal(true); } },
                  { label: "rate an album", desc: "post a review with a score", action: () => { setShowCreateSheet(false); setShowQuickReviewModal(true); } },
                  { label: "share a mix", desc: "feature a set of albums", action: () => { setShowCreateSheet(false); setShowShareMixModal(true); } },
                ].map((opt) => (
                  <button key={opt.label} onClick={opt.action} style={{ display: "block", width: "100%", textAlign: "left", padding: "14px 16px", background: "none", border: "none", borderBottom: `1px solid ${LINE}`, cursor: "pointer", fontFamily: "inherit" }}>
                    <div style={{ fontSize: 14, fontWeight: 400, color: INK }}>{opt.label}</div>
                    <div style={{ fontSize: 11.5, color: MUTE, marginTop: 1 }}>{opt.desc}</div>
                  </button>
                ))}
              </div>
            </div>
          )}
          {/* Per handoff spec "Bottom nav bar — 2c": text labels (no icons), navy
              accent rule for selected, outlined + post action in slot 3. */}
          <div style={{ position: "fixed", left: 0, right: 0, bottom: 0, zIndex: 80, background: BG, borderTop: `1px solid ${LINE}`, display: "grid", gridTemplateColumns: "repeat(5, 1fr)", alignItems: "stretch", padding: "16px 10px calc(env(safe-area-inset-bottom, 0px) + 8px)" }}>
            {[
              ...
              <button key={t.key} onClick={() => { if (t.active) window.scrollTo({ top: 0, behavior: "smooth" }); else t.go(); }} className="ui-sans" style={{ background: "none", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 7, padding: 0, fontFamily: "inherit" }}>
                ...
                <span style={{ width: 26, height: 2, background: t.active ? BLUE : "transparent" }} />
              </button>
            ))}
          </div>
        </>
      ...
      <div style={{ maxWidth: 920, margin: "0 auto", padding: ((view.name === "profile" && (profile.profileTheme === "mspaint" || (profile.profileTheme === "bubble" && profile.username === ADMIN_USERNAME))) || (view.name === "userProfile" && viewedUser?.profileTheme === "mspaint")) ? 0 : "28px 16px 64px", minHeight: "calc(100vh - 60px)", position: "relative", background: (view.name === "profile" ? (profile.pageBackground || "") : view.name === "userProfile" ? (viewedUser?.pageBackground || "") : "") || undefined, boxShadow: (view.name === "profile" ? (profile.pageBackground || "") : view.name === "userProfile" ? (viewedUser?.pageBackground || "") : "") ? `0 0 0 100vmax ${(view.name === "profile" ? profile.pageBackground : viewedUser?.pageBackground) || "transparent"}` : undefined, clipPath: (view.name === "profile" ? (profile.pageBackground || "") : view.name === "userProfile" ? (viewedUser?.pageBackground || "") : "") ? "inset(0 -100vmax)" : undefined, transition: "background 0.2s" }}>
        ...
        {view.name === "activity" && (
          <div>
            ...
            <ActivityFeed
              ...
              onFilter={(f) => { setActivityFilter(f); setActivityCursor(null); loadActivity(view.username, f); }}
        ...
        {view.name === "profile" && profile.profileTheme === "bubble" && profile.username === ADMIN_USERNAME && (<>
          <BubbleProfile
            ...
            mixes={albumMixes.filter((m) => m.isPublic !== false).map((m) => ({ id: m.id, title: m.title, count: m.albums.length, cover: <MixCoverStack albums={m.albums} fetchedAlbums={fetchedAlbums} albumById={albumById} size={56} /> }))}
              ...
              <div style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", alignItems: "flex-end", flexShrink: 0, gap: 16 }}>
                ...
                {!isMobile && (
                  <div style={{ display: "flex", gap: 32, alignItems: "flex-start", flexShrink: 0, marginRight: 24 }}>
                    ...
                    <Stat label="activity" value={<PencilGlyph size={20} />} onClick={() => openActivity(profile.username, profile.id, true)} />
...
function BubbleProfile({ displayName, username, bio, isMobile, palette, stats, albums, reviews, info, mixes, queued,
  ...
  React.useEffect(() => {
    ...
    queued.forEach((a) => { if (a && (!a.album || a.album.title === "Unknown Album")) onNeedAlbum(a.id); });
  }, [albums, reviews, queued, onNeedAlbum]);

  const pal = AERO_PALETTES[palette] || AERO_PALETTES.aqua;
  ...
  const Cover = ({ item }) => (
    <div style={{ padding: 4, borderRadius: 6, background: "linear-gradient(180deg,rgba(255,255,255,.9),rgba(255,255,255,.4))", boxShadow: "0 4px 12px rgba(0,20,50,.28)" }}>
      <div style={{ aspectRatio: "1/1", borderRadius: 3, overflow: "hidden", background: pal.ink }}>{renderAlbumCover(item)}</div>
    </div>
  );
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1000, margin: "0 auto", padding: `${pad}px ${pad}px ${pad + 30}px` }}>
        ...
        <div style={{ display: "flex", gap: isMobile ? 16 : 26, alignItems: "center" }}>
          ...
          <div style={{ minWidth: 0, flex: 1, textAlign: "left" }}>
            <div style={{ fontSize: isMobile ? 30 : 46, fontWeight: 700, color: "#fff", letterSpacing: "-.02em", lineHeight: 1, textShadow: NAME_SH, overflowWrap: "anywhere", textAlign: "left" }}>{displayName || username}</div>
            <div style={{ fontSize: isMobile ? 15 : 18, color: "rgba(255,255,255,.85)", textShadow: WHITE_SH, marginTop: 6, textAlign: "left" }}>@{(username || "").toLowerCase()}</div>
        ...
        {/* info (left) nestled next to recent reviews (right) */}
        <div style={{ display: isMobile ? "block" : "flex", gap: 16, marginTop: 16, alignItems: "flex-start" }}>
          <div style={{ ...frosted, flex: isMobile ? "auto" : "0 0 42%", marginBottom: isMobile ? 16 : 0 }}>
            ...
            {[["Age", info.age], ["Town", info.town], ["Country", info.country], ["Interests", info.interests], ["Mood", info.mood ? (AERO_MOOD[info.mood] || info.mood) : ""]].map(([label, val], i) => (
              <div key={label} style={{ display: "flex", padding: isMobile ? "9px 15px" : "11px 18px", fontSize: isMobile ? 14 : 15, borderTop: i === 0 ? "none" : "1px solid rgba(255,255,255,.35)" }}>
                <div style={{ width: isMobile ? 96 : 96, flexShrink: 0, fontWeight: 700, color: pal.ink }}>{label}</div>
                <div style={{ color: "#12384f", overflowWrap: "anywhere" }}>{val || "—"}</div>
              </div>
            ))}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div key={i} onClick={() => onOpenAlbum(r.albumId)} style={{ ...frosted, background: "linear-gradient(180deg,rgba(255,255,255,.34),rgba(255,255,255,.14))", padding: isMobile ? 12 : 15, display: "flex", gap: isMobile ? 12 : 16, cursor: "pointer" }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap" }}>
                      <span style={{ fontSize: isMobile ? 20 : 26, fontWeight: 700, color: pal.ink }}>{r.score}/10</span>
                      <span style={{ fontSize: isMobile ? 14 : 16, fontWeight: 700, color: "#12384f", overflowWrap: "anywhere" }}>{r.albumTitle}</span>
                      {r.artist && <span style={{ fontSize: isMobile ? 12 : 14, color: "#3f6a86" }}>{r.artist}</span>}
                    </div>
                    {r.body && <div style={{ fontSize: isMobile ? 13 : 13.5, lineHeight: 1.5, color: "#14384e", marginTop: 5 }}>{r.body}</div>}
                  </div>
                </div>
              ))}
              {reviews.length === 0 && <div style={{ color: "rgba(255,255,255,.92)", textShadow: WHITE_SH, fontSize: 14 }}>no reviews yet.</div>}
            </div>
          </div>
        </div>
        ...
        {mixes.length > 0 && (
          <div style={{ ...frosted, marginTop: 16 }}>
            <Bar>mixes</Bar>
            <div style={{ display: "flex", flexDirection: "column", gap: 10, padding: isMobile ? 12 : 16 }}>
              {mixes.map((m) => (
                <div key={m.id} onClick={() => onOpenMix(m.id)} style={{ display: "flex", alignItems: "center", gap: 12, cursor: "pointer", background: "rgba(255,255,255,.22)", border: "1px solid rgba(255,255,255,.5)", borderRadius: 10, padding: "10px 12px" }}>
                  <div style={{ flexShrink: 0, lineHeight: 0 }}>{m.cover}</div>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: isMobile ? 14 : 15, fontWeight: 700, color: "#12384f", overflowWrap: "anywhere" }}>{m.title}</div>
                    <div style={{ fontSize: 12, color: "#3f6a86", marginTop: 2 }}>{m.count} album{m.count !== 1 ? "s" : ""}</div>
                  </div>
                </div>
              ))}
            </div>
...
function LandingPage({ onAuthed }) {
  ...
  const wordmark = (nameSize) => (
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      <img src="/spindex-logo.svg" alt="noteblock" style={{ width: 30, height: 30, objectFit: "contain" }} />
    </div>
  );
    ...
    return (
      <div style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", background: NB.page, minHeight: "100vh", display: "flex", flexDirection: "column", color: NB.ink }}>
        ...
        <div style={{ padding: "30px 22px 16px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", gap: 14 }}>
          ...
          <h1 style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", fontWeight: 700, color: "#000000", fontSize: 34, lineHeight: 1.0, letterSpacing: "-0.03em", margin: 0 }}>keep a record of<br/>every record</h1>
          <p style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", fontSize: 11, lineHeight: 1.4, color: NB.body, margin: 0, textAlign: "center", whiteSpace: "nowrap" }}>review albums, discover new music, and connect with others</p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.15fr 1fr", minHeight: 760, width: "100%", maxWidth: 1160, border: `1px solid ${NB.border}` }}>
        <div style={{ display: "flex", flexDirection: "column", borderRight: `1px solid ${NB.hair}`, overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", padding: "40px 44px 0" }}>
            ...
            <h1 style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", fontWeight: 700, color: "#000000", fontSize: 64, lineHeight: 0.95, letterSpacing: "-0.035em", margin: "18px 0 0" }}>keep a record of<br/>every record</h1>
            <p style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", fontSize: 15, lineHeight: 1.5, color: NB.body, margin: "18px 0 0", textAlign: "center", whiteSpace: "nowrap" }}>review albums, discover new music, and connect with others</p>
          </div>
